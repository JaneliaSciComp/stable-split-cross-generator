...
def convert_vt(search_term, vtcache = None):
    if vtcache != None:
...
def process_input(a_line):
    ...
    # Convert A line
    converted_aline = a_line.upper()
    if a_line:
        original = a_line.rstrip()
        if original.isdigit():
...
def runprogram(split_name, line_names, a_line):
    initialize_program()
    logger = colorlog.getLogger()
    if a_line and a_line != '':
        CROSSES = open(a_line + '-' + split_name + '.crosses.txt', 'w')
        FLYCORE = open(a_line + '-' + split_name + '.flycore.xls', 'w')
        nocross_file = a_line + '-' + split_name + '.no_crosses.txt'
        NO_CROSSES = open(nocross_file, 'w')
    else:
        CROSSES = open(split_name + '.crosses.txt', 'w')
        FLYCORE = open(split_name + '.flycore.xls', 'w')
        nocross_file = split_name + '.no_crosses.txt'
        NO_CROSSES = open(nocross_file, 'w')
